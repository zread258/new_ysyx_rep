...
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  if (in_pmem(left) || in_pmem(right)) {
    if (addr_is_rom(addr)) report_mmio_overlap(name, left, right, "mrom", MROM_LEFT, MROM_RIGHT);
    if (addr_is_sram(addr)) report_mmio_overlap(name, left, right, "sram", SRAM_LEFT, SRAM_RIGHT);
