...
/* The following contents are all testing codes. They have been already commented.


  // if (!fp) {
  //   fp = fopen("/home/csardas/ysyx-workbench/nemu/tools/gen-expr/build/input", "r");
  //   assert(fp);
  // }
  ...
  // line_read = malloc(strlen(line) + 1);
  // strcpy(line_read, line);

  // Ans = strtol(ans, NULL, 10);
...
static int cmd_p(char *args) {
  if (args == NULL) {
    ...
    // if (Ans == result) {
    //   printf("Match! Ans = %u result = %u\n", Ans, result);
    // } else {
    //   printf("Not Match! Ans = %u result = %u\n", Ans, result);
    // }
    */
