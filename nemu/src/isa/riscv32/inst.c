...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (word_t)((SEXT(src1, 32) * src2) >> 32));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ (SEXT(imm, 20)));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> imm);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | (SEXT(imm, 20)));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & (SEXT(imm, 20)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm; IFDEF(CONFIG_FTRACE, { 
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, if (rd != 0) R(rd) = csr(imm); csr(imm) |= src1);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc += imm - 4;  IFDEF(CONFIG_FTRACE, { 
...
int isa_exec_once(Decode *s) {
  ...
  IFDEF(CONFIG_RING_ITRACE, itrace_push(s->pc, s->isa.inst.val));
